...
$PRODUCTION = true; // Set to true in production
...
define('SMS_HOST', '10.29.244.212:8080');
// define('SMS_PORT', '8080');
define('SMS_USER', 'sms');
define('SMS_PASS', '8ZrM1BXc');
define('SMS_PATH', '/messages');

define('CALL_HOST', '10.29.244.212:8084');
...
$open_actions = ['login', 'logout', 'report_alert', 'session','record_reading', 'dashboard_stream', 'subscribe_push', 'list_push_subscriptions'];
...
$bfp_actions = ['bfp_dashboard', 'stations', 'incident_trends', 'incidents', 'device_details', 'resolve_alert', 'get_users', 'add_device'];
...
switch ($action) {
    ...
    case 'stations':
        ...
        if ($response['status'] === 'success') respond($response['data'] ?? ['message' => $response['message'], 'id' => $response['station_ID'] ?? null]);
