...
function fetchDashboardData($pdo) {
  ...
  $stmt_users = $pdo->query("
    ...
    GROUP BY u.user_ID, u.first_name, u.last_name, u.phone_number, l.address, l.location_name
  ...
  foreach ($users as &$user) {
    ...
    // Use the location name from the first location joined.
    // If the user has multiple locations, the one with the smallest ID will typically be chosen by MySQL's GROUP BY behavior.
  ...
  // Add derived incident_type for active incidents
  ...
  foreach ($resolvedIncidents as &$i) {
    ...
    // Attach server-derived incident_type when possible
...
function handleUserCrud($pdo, $method, $data) {
  ...
  // POST - Create User (Placeholder for complexity)
  if ($method === 'POST') {
  ...
  if ($method === 'PUT') {
    ...
    try {
      ...
      $stmt_user->execute([
        ...
        $data['email'] ?? null,
        $data['role'] ?? 'resident', // Assuming default is resident if not passed
        $user_id
      ]);
      ...
      if (isset($data['address']) || isset($data['location_name'])) {
           $stmt_location = $pdo->prepare("
              ...
              LIMIT 1
            ");
            // NOTE: If no location exists, this silently fails, a POST/CREATE location should handle that.
...
case 'users':
  ...
  case 'resolve_alert':
    // IMPORTANT: Temporarily setting required role to allow either officer or desk to access dashboard features
    $is_bfp_role = isset($_SESSION['role']) && ($_SESSION['role'] === 'bfp_officer' || $_SESSION['role'] === 'bfp_assigned_at_desk');
    if (!$is_bfp_role) {
        http_response_code(401);
        $response = ['status' => 'error', 'message' => 'Authentication required.'];
    ...
    // Fixed: Removed incorrect nested endpoint checks
    if ($method === 'PUT' && isset($input['incident_ID'])) {
      ...
      $response = ['status' => 'success', 'message' => 'Incident resolved.'];
    } else {
      http_response_code(400);
      $response = ['status' => 'error', 'message' => 'Invalid request for resolve_alert.'];
    }
    break;
